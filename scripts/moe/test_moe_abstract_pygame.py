#!/usr/bin/env python3
"""
Pygame visualization for MoE Abstract (entity-based) model on KeyDoor.
...
def resize_window(grid_size: int):
    ...
    pygame.display.set_caption(f"MoE Abstract Model Test - KeyDoor Environment ({grid_size}x{grid_size})")
...
def act_from_entities(model: MoEAbstract, obs, device):
    ...
    with torch.no_grad():
        logits, gate_weights = model(ent_tensor, mask_tensor)
        action = torch.argmax(logits, dim=-1).item()
    return action, logits


def test_moe_abstract_pygame():
    print("🎮 Testing MoE Abstract Model with Pygame Visualization")
    print("=" * 60)
    ...
    # Load model
    model_path = project_root / "experiments" / "exp_moe_abstract_keydoor_seed0" / "moe_abstract_final.pth"
    model = MoEAbstract(n_experts=4, entity_dim=10, max_entities=16, n_actions=6, hidden_dim=128, device=str(device))
    model.to(device)
    model.eval()
    if not model_path.exists():
        print(f"❌ Model not found at: {model_path}")
        print("Please train the model first using: python scripts/train_moe_abstract.py")
        return
    ...
    stuck_threshold = 40  # Increased to give agents more time to explore
